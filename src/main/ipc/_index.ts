...
import { fbLogOut } from "./fb-log-out";
import { getFBMarketListings } from "./get-fb-market-listings";
...
ipcMain.handle(IPC_EVENTS.FB_LOG_OUT, fbLogOut);

ipcMain.handle(IPC_EVENTS.GET_FB_MARKET_LISTINGS, async (_, settings) => {
  return await getFBMarketListings(settings);
});
